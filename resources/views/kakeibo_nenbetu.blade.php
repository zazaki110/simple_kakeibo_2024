...
<body>
  ...
  <table class="table table-striped table_nenbetu">
    ...
    <tr>
      <td>{{$counter}}</td>
      <td>{{ $syuturyoku->payment_name }}
      <td>{{number_format( $syuturyoku->spending )}}円
      <td>{{ number_format($syuturyoku->income) }}円
      <td>{{ $syuturyoku->created_at}}</td>
      <td><a href="{{url('edit_screen_kakeibo',['id'=>$syuturyoku->id,'naiyou'=>$syuturyoku->payment_name,'sisyutu'=>$syuturyoku->spending,'syunyuu'=>$syuturyoku->income,'nitizi'=>$syuturyoku->created_at])}}">編集</a>|<a href="{{url('delete_kakeibo',['id'=>$syuturyoku->id])}}">消去</a></td>
    </tr>
    @endforeach
    @endforeach
    @endforeach
    ...
    <div class="syusi_waku">
      <div class="sisyutu_goukei">支出合計</div>
      <div class={{$total_sisyutu_name}}>{{number_format($sisyutgoukei->total_spending)}}円</div>
    </div>
    <div class="syunyu_goukei">収入合計</div>
    <div class={{$total_syunyu_name}}>{{number_format($syunyugoukei->total_income)}}円</div>

    <div class="syusi">収支</div>
    <div class={{$syusi_name}}>{{number_format($syunyugoukei->total_income-$sisyutgoukei->total_spending)}}円</div>
    ...
    <form method="post" action="{{ url('data_search_seireki') }}">
      {{ csrf_field() }}
      <div class={{$karenda}}><input type="number" name="seireki" id="nenbetu_seireki" value={{$seireki}} onchange="ischeck5(this)" required></div> <!-- ユーザが検索フォームから値を入力して年別にデータを取得できるようにする -->
    ...
    <form method="post" action="{{ url('data_search_seireki_add') }}">
      {{ csrf_field() }}
    ...
    <form method="post" action="{{ url('data_search_seireki_previous_year') }}">
      {{ csrf_field() }}
